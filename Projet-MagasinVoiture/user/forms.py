...
class ArticleForm(ModelForm):
    class Meta:
        model = Article
        labels = {"name": "Nom de l'article", "date_ajout": "Date d'ajout de l'article",
                  "ville": "Ville de l'article", "piece": "Type de la pièce", "article_image": "L'image de l'article"}
        ...
        if 'ville' in self.data:
            try:
                ...
                self.fields['piece'].queryset = Piece.objects.filter(
                    ville_id=ville_id).order_by('name')
            except (ValueError, TypeError):
                pass
        elif self.instance.pk:
            self.fields['piece'].queryset = self.instance.ville.piece_set.order_by(
                'name')
...
class VilleForm(ModelForm):
    class Meta:
        model = Ville
        labels = {"name": "Nom de la ville",
                  "describtion": "Description de la ville"}
        fields = "__all__"
...
class PieceForm(ModelForm):
    class Meta:
        model = Piece
        labels = {"name": "Nom",
                  "describtion": "Description du type de la pièce"}
        fields = "__all__"
